...
/*
 * Anotation：
 * <1>rt_thread_priority_table 线程优先级表，数组链表的形式，高优先级对应高下标地址。
 * <2>rt_thread 当前正在的线程
 * <3>rt_thread_defunct 当前的服务程序
 * */
...
/*
 * Anotation：三个线程钩子，在线程挂起、恢复、初始化时会被调用，输出相关调试信息。
 *  rt_thread_suspend_hook 在 rt_thread_suspend中被调用。
...
void rt_thread_exit(void)
{
     ...
     * Anotation：使用数组+双向循环链表的好处就是，在循环链表中，对于任何节点都是有相同的性质。而且在对某一个节点进行删除操作时有统一的接口：即双向循环链表节点的删除。
     * << 还有就是，通过数组下标来表示线程的优先级，这样可以通过下标直接访问，即随机访问的方式，这样有较快的查询，也就是在调度的时候有较高的效率。
     * */
...
/*
 * Anotation：线程对象的构造函数：包括线程服务函数，栈空间和地址的分配，以及线程定时器和时间片的初始化。
 * */
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 *
 * Anotation：线程启动，主要是将线程加入调度器，由调度器开始调度。
 */
rt_err_t rt_thread_startup(rt_thread_t thread)
{
    ...
    /* then resume it
     * Anotation：在一个新线程开始调度的过程，也就和一个老现场由其他现场状态恢复调度的过程一致，所以这里使用现成的接口。
     * */
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 *
 * Anotation：脱机一个线程：包括从线程队列移除和脱离系统对象的管理
 */
rt_err_t rt_thread_detach(rt_thread_t thread)
{
    ...
    /*
     * Anotation：
     * <1>_thread_cleanup_execute 从线程队列移除
     * <2> rt_timer_detach
     * */
    _thread_cleanup_execute(thread);
 ...
 * @return RT_EOK
 *
 * Anotation：使用此延时，并不是死循环，不同于裸机的delay，这里仅对于本线程进入睡眠，一段时间tick过后，进入正常的系统调度运行。
 */
 ...
 * @return RT_EOK
 *
 * Anotation：在原有睡眠时间的基础加上一部分的延时操作。
 */
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 *
 * Anotation：将线程从挂起的链表中移除，并将其添加到调度器中开始调度运行。
 */
rt_err_t rt_thread_resume(rt_thread_t thread)
{
    ...
    // Rt-thread 中使用 状态变量8位 + Mast掩码的形式，实现了 1位表示一个状态变量。
 ...
 *
 * Anotation：线程时间片用完之后调用此函数进行下一个线程的调度。
 */
