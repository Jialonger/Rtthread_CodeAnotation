...
static rt_tick_t rt_timer_list_next_timeout(rt_list_t timer_list[])
{
    ...
    {
        /*
         * rt_list_entry 从一个结构体成员指针获取指向其结构体的指针。
         * */
 ...
 *
 * Anotation：rt_timer_t由rt_object_t继承而来，所以在rt_object_init过程中进行了向上转型的操作，这是多态的一种实现。
 */
 ...
 *
 * Anotation：从内核移除定时器，定时器的析构函数。构造和析构的调用顺序相反，所以在这个析构函数中包含了rt_object_detach函数进行析构。
 */
                           ...
                           rt_uint8_t  flag)
{
    ...
    /* allocate a object
     *
     * rt_object_allocate 是所有对象的一个内存管理函数，是一个分配器，类似于STL中 allocator给所有的container进行内存分配一样
     * */
    ...
    /*
     * Anotation：调用定时器部分的初始化函数，带有下划线开头的函数表示是对象的成员函数。
     * */
    _rt_timer_init(timer, timeout, parameter, time, flag);
...
rt_err_t rt_timer_delete(rt_timer_t timer)
{
    ...
    level = rt_hw_interrupt_disable();
    /*
     * Anotation：调用定时器部分的移除函数，带有下划线开头的函数表示是对象的成员函数。
    */
    _rt_timer_remove(timer);
    /* stop timer
     *
     * Anotation：rt_object 中的 flag对于不同的子类对象有不同的flag含义。
     * */
    timer->parent.flag &= ~RT_TIMER_FLAG_ACTIVATED;
 ...
 * @return the operation status, RT_EOK on OK, -RT_ERROR on error
 *
 * Anotation：启动定时器，内核的软件和硬件定时器都开始运行。
 */
