 ...
 *
 *
 * Anotation：系统支持的一些组件，主要包括系统初始化的宏，
 */
...
void rt_components_board_init(void)
{
    ...
    /*
     * Anotation： volatile 关键词：volatile修饰的变量每次都要到内存中获取后再使用，而不是直接使用寄存器中临时数据；
     * << 意味着这个变量很容易改变，如果使用寄存器中的临时数据可能与其内存中的数据不对应，导致逻辑上的错误。
     * */
    volatile const init_fn_t *fn_ptr;
    /*
     * Anotation：循环调用注册的初始化函数。
     * */
...
int rtthread_startup(void)
{
     ...
     * Anotation：板载硬件的初始化，硬件作为最底层，所以最先初始化。
     */
    rt_hw_board_init();
    ...
    /*
     * Anotation：调度器使用到了定时器，所以要先初始化定时器。
     * */
