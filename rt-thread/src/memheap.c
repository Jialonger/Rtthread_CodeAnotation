 ...
 *
 * Reference：https://zhuanlan.zhihu.com/p/46001988
 * Anotation：
 */
...
/*
 * Anotation：作用于本文件的一个全局内存堆变量，是整个系统内存分配的源头。
 * */
...
/*
 * Anotation：系统所有的动态内存分配都从这个函数实现，从而内核所有的内存分配由内存堆实现
 * */
...
/*
 * Anotation：系统所有的动态内存回收都从这个函数实现，从而内核所有的内存分配由内存堆实现
 * */
...
/*
 * Anotation：系统所有的动态内存重新分配都从这个函数实现，从而内核所有的内存分配由内存堆实现
 * */
